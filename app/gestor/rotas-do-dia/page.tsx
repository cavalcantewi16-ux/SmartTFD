              ...
              <div className="flex gap-2 overflow-x-auto pb-2">
                {rota.pacs.map(pac=>(
                  <div key={pac._uid} draggable onDragStart={()=>setDragging({ru:rota._uid,pu:pac._uid})} onDragOver={e=>{e.preventDefault();setDragOver({ru:rota._uid,pu:pac._uid})}} onDrop={e=>{e.preventDefault();onDropOnPac(rota._uid,pac._uid)}} onDragEnd={()=>{setDragging(null);setDragOver(null)}} className={`bg-white rounded-xl border p-3 min-w-[170px] w-44 flex-shrink-0 space-y-2 text-xs cursor-grab transition-opacity ${dragOver?.pu===pac._uid&&dragging?.pu!==pac._uid?"border-blue-400 border-2":"border-gray-200"} ${dragging?.pu===pac._uid?"opacity-40":""}`}>
                    ...
                    {pac.lat_gestor&&pac.lng_gestor&&(<div className="bg-blue-50 border border-blue-200 rounded px-1.5 py-1 text-xs text-blue-700 flex items-center justify-between gap-1"><span>Loc. salva pelo gestor</span><span className="font-mono opacity-70">{pac.lat_gestor?.toFixed(4)}, {pac.lng_gestor?.toFixed(4)}</span></div>)}{pac.lat&&pac.lng&&(<div className="bg-green-50 border border-green-200 rounded px-1.5 py-1 text-xs text-green-700 flex items-center justify-between gap-1"><span>Loc. salva pelo motorista</span><span className="font-mono opacity-70">{pac.lat?.toFixed(4)}, {pac.lng?.toFixed(4)}</span></div>)}<p className="text-[10px] text-gray-400 mt-2 mb-0.5">Destino</p><div className="flex items-center gap-1"><select value={pac.hospital_id} onChange={e=>setPac(rota._uid,pac._uid,{hospital_id:e.target.value})} className="flex-1 border rounded px-1 py-1">
