...
function tempoEstimadoMin(distKm: number) {
  return Math.ceil((distKm * 1.45) / 68 * 60)
}

function tempoParaHospital(h: { lat?: number | null; lng?: number | null; cidade?: string | null }): number {
  if (h.lat && h.lng) {
    const dist = haversineKm(BOQUEIRAO.lat, BOQUEIRAO.lng, h.lat, h.lng)
    return tempoEstimadoMin(dist)
  }
  if (h.cidade) {
    const key = h.cidade.toLowerCase()
    for (const [nome, tempo] of Object.entries(TEMPOS_CIDADE)) {
      if (key.includes(nome.trim())) return tempo
    }
  }
  return 120
}
...
export async function POST(req: NextRequest) {
  ...
  const { items, vehicles } = body as { items: PlanningItem[]; vehicles: Vehicle[] }

  if (!items?.length) return NextResponse.json({ error: 'Nenhum item informado' }, { status: 400 })
  if (!vehicles?.length) return NextResponse.json({ error: 'Nenhum veículo informado' }, { status: 400 })

  const invalidos = items.filter(i => !i.patient_name?.trim() || !i.hospital_id || !i.consultation_time)
  if (invalidos.length) {
    return NextResponse.json({
      error: `${invalidos.length} item(s) incompleto(s) — preencha nome, destino e horário`,
    }, { status: 400 })
  }
  ...
  for (const [hospId, hospItems] of porHospital) {
    ...
    for (let i = 1; i < sorted.length; i++) {
      if (timeToMin(sorted[i].consultation_time) - anchorMin <= JANELA_AGRUP_MIN) {
        janela.push(sorted[i])
      } else {
  ...
  const gruposComTempo = grupos.map(g => {
    ...
    const saidaMin = timeToMin(g.earliest_time) - tempoViagem - BUFFER_MIN
    return { ...g, tempoViagem, saidaMin }
  ...
  const resultado = gruposComTempo.map((grupo) => {
    const { tempoViagem, saidaMin, ...g } = grupo
    ...
    if (veiculo && g.total_pessoas > veiculo.capacidade) {
      avisos.push(`⚠️ Grupo tem ${g.total_pessoas} pessoas, veículo tem ${veiculo.capacidade} lugares`)
    }
