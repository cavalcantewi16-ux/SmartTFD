...
function StatusBadge({ status }: { status: string }) {
  if (status === 'embarcou') return <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium">â Embarcou</span>
  if (status === 'ausente') return <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full font-medium">â Ausente</span>
...
export default function MotoristaPage() {
  ...
  async function enviarProblema() {
    ...
    alert('â Problema reportado ao gestor.')
  }
      ...
      <div className="bg-blue-800 text-white px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-xs">
          <span className={gpsAtivo ? 'text-green-300' : 'text-gray-400'}>
            {gpsAtivo ? 'ð GPS ativo' : 'ð sem GPS'}
          </span>
  ...
  function BotaoLocalizacao({ passenger }: { passenger: Passenger }) {
    ...
    if (estado === 'loading') return <span className="text-xs bg-blue-50 text-blue-500 px-2 py-1 rounded-lg animate-pulse">ð Salvandoâ¦</span>
    if (estado === 'ok') return <span className="text-xs bg-green-50 text-green-600 px-2 py-1 rounded-lg">â Local salvo!</span>
    if (estado === 'err') return <span className="text-xs bg-red-50 text-red-600 px-2 py-1 rounded-lg">â Sem GPS</span>
    return (
      <button onClick={() => salvarLocalizacaoPaciente(passenger.paciente.id, passenger.paciente.nome)}
        ...
        {temGps ? 'ð Atualizar local' : 'ð Salvar local'}
      </button>
    )
  }
  ...
  if (!loading && !plan) return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center gap-4">
        ...
        <button onClick={() => carregar()} className="mt-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm">ð Verificar novamente</button>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center gap-4">
        ...
        <p className="text-gray-400 text-sm">
          VocÃª concluiu {legs.length} viagem(ns) hoje.<br /> Bom descanso! ð
        </p>
        <div className="bg-white rounded-xl shadow p-4 text-left w-full max-w-xs space-y-2 mt-2">
          {legs.map((l, i) => (
            <div key={l.id} className="flex items-center gap-2 text-sm">
              <span className="text-green-500">â</span>
              <span className="text-gray-700">Viagem {i + 1} â {l.hospital.nome}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <Header />
      <div className="flex-1 flex items-center justify-center text-gray-400">Carregando planoâ¦</div>
    </div>
  )
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col pb-24">
      ...
      {legAtual && (
        <div className="bg-blue-700 text-white px-4 py-2 text-sm flex items-center justify-between">
          <span className="font-semibold">Viagem {legIdx + 1} de {legs.length} Â· {legAtual.hospital.nome}</span>
      ...
      <div className="flex-1 p-4 space-y-4">
        {legAtual?.status === 'aguardando' && (
          <>
            <div className="bg-white rounded-2xl shadow p-5 text-center space-y-3">
              <div className="text-gray-400 text-sm">{legIdx === 0 ? 'Primeira saÃ­da em' : 'PrÃ³xima saÃ­da em'}</div>
              ...
              <div className="text-sm text-gray-600">ð¥ {legAtual.hospital.nome}{legAtual.hospital.cidade && ` â ${legAtual.hospital.cidade}`}</div>
            ...
            <div className="bg-white rounded-2xl shadow p-4">
              <h3 className="font-semibold text-gray-700 text-sm mb-3">ð¥ {legAtual.passengers.length} parada(s) nesta viagem</h3>
                ...
                {legAtual.passengers.map((p, i) => (
                  <div key={p.id} className="flex items-start gap-3 text-sm">
                    ...
                    <div>
                      ...
                      {p.est_pickup_at && <div className="text-xs text-blue-600">ð {fmtHora(p.est_pickup_at)}</div>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <>
            {passeiroAtual && !todosHandled && (
              <div className="bg-blue-600 text-white rounded-2xl shadow p-5 space-y-3">
                <div className="text-blue-200 text-xs font-medium uppercase tracking-wide">PrÌ³{xima parada</div>
                ...
                {(passeiroAtual.paciente.endereco || passeiroAtual.paciente.bairro) && (
                  <div className="text-blue-100 text-sm">ð {[passeiroAtual.paciente.endereco, passeiroAtual.paciente.bairro].filter(Boolean).join(', ')}</div>
                )}
                {passeiroAtual.est_pickup_at && <div className="text-blue-200 text-sm">ð Estimado: {fmtHora(passeiroAtual.est_pickup_at)}</div>}
                ...
                <div className="pt-1 border-t border-white/20">
                  <div className="text-blue-200 text-xs mb-1.5">VocÃª estÃ¡ em frente Ã  casa do paciente?</div>
                  ...
                  return (
                    <div key={p.id} className={`flex items-start gap-3 p-3 rounded-xl transition-colors ${isCurrent ? 'bg-blue-50 border border-blue-200' : ''} ${p.status === 'embarcou' ? 'opacity-60' : ''} ${p.status === 'ausente' ? 'opacity-40' : ''}`}>
                      <span className={`w-6 h-6 rounded-full text-xs flex items-center justify-center flex-shrink-0 mt-0.5 font-bold ${p.status === 'embarcou' ? 'bg-green-100 text-green-700' : p.status === 'ausente' ? 'bg-red-100 text-red-600' : isCurrent ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
                        {p.status === 'embarcou' ? 'â' : p.status === 'ausente' ? 'â' : i + 1}
                      </span>
              ...
              <div className="space-y-1.5">
                ...
                {legAtual.passengers.filter(p => p.status === 'embarcou').map(p => (
                  <div key={p.id} className="flex items-center gap-2 text-sm bg-green-50 rounded-lg px-3 py-2">
                    <span className="text-green-600">â</span><span className="text-gray-700">{p.paciente.nome}</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="space-y-1.5">
                ...
                {legAtual.passengers.filter(p => p.status === 'ausente').map(p => (
                  <div key={p.id} className="flex items-center gap-2 text-sm bg-red-50 rounded-lg px-3 py-2">
                    <span className="text-red-500">â</span><span className="text-gray-500">{p.paciente.nome}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {legAtual?.status === 'retornando' && (
          <div className="space-y-4">
            <div className="bg-white rounded-2xl shadow p-5 text-center space-y-3">
              ...
              <h2 className="font-bold text-gray-800">Retornando Ã  cidade</h2>
            ...
            {proxLeg && (
              <div className="bg-blue-50 rounded-2xl p-4 space-y-2 border border-blue-100">
                <div className="text-xs text-blue-500 font-semibold uppercase tracking-wide">PrÃ³xima viagem (Viagem {legIdx + 2})</div>
                <div className="font-semibold text-gray-800 text-sm">ð¥ {proxLeg.hospital.nome}</div>
                <div className="text-xs text-gray-500">SaÃ­da: {fmtHora(proxLeg.est_departure_at, proxLeg.horario_saida)}{proxLeg.est_outbound_min && ` Â· ~${proxLeg.est_outbound_min} min`}</div>
        ...
        {legs.filter(l => l.status === 'concluida').length > 0 && legAtual && (
          <div className="bg-white rounded-2xl shadow p-4">
            <h3 className="text-xs text-gray-400 font-medium uppercase tracking-wide mb-2">ConcluÃ­das</h3>
            {legs.filter(l => l.status === 'concluida').map((l, i) => (
              <div key={l.id} className="flex items-center gap-2 text-sm py-1.5 border-b last:border-0">
                ...
                <span className="text-gray-600">Viagem {i + 1} â {l.hospital.nome}</span>
      ...
      {plan && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-3">
          <button onClick={() => setModalProblema(true)}
            ...
            ð§ Reportar problema no veÃ­culo
          </button>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end">
          <div className="bg-white w-full rounded-t-2xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-gray-800">ð§ Problema no VeÃ­culo</h3>
              <button onClick={() => setModalProblema(false)} className="text-gray-400 text-xl">â</button>
            </div>
            <div>
            ...
            <div>
              <label className="text-xs text-gray-500 mb-1 block">UrgÃªncia</label>
            ...
            <div>
              <label className="text-xs text-gray-500 mb-1 block">DescriÃ§Ã£o</label>
              <textarea value={formProblema.descricao} rows={3} onChange={e => setFormProblema(f => ({ ...f, descricao: e.target.value }))} placeholder="Descreva o problemaâ¦" className="w5full border rounded-xl px-3 py-2.5 text-sm resize-none" />
            </div>
            <button onClick={enviarProblema} disabled={enviandoProbl || !formProblema.descricao.trim()} className="w5full bg-orange-500 text-white py-3 rounded-xl font-bold disabled:opacity-40 active:scale-95 transition-transform">
              {enviandoProbl ? 'Enviandoâ¦' : 'ð Reportar ao Gestor'}
            </button>
          </div>
        </div>
      )}
