...
export default function MotoristaPage() {
  ...
  const carregar = useCallback(async (uid?: string) => {
    ...
    const { data } = await supabase
      ...
      .select(`
        ...
        route_legs(
          id, horario_saida, ordem, status,
          est_departure_at, est_hospital_at, est_return_at,
          est_outbound_min, est_return_min,
          hospital:hospitais(id, nome, cidade, lat, lng),
          leg_passengers(
            id, ordem, status, est_pickup_at,
            paciente:pacientes(id, nome, endereco, bairro, lat, lng)
          )
        )
      `)
      .eq('data', hoje)
    ...
    if (data) {
      const legsOrdenados: Leg[] = ((data.route_legs as any[]) || [])
        ...
        .map((l: any) => ({
          ...l,
          passengers: (l.leg_passengers || []).sort((a: any, b: any) => a.ordem - b.ordem),
        }))
  ...
  async function enviarProblema() {
    ...
    await supabase.from('manutencao_veicular').insert({
      veiculo_id:   (plan.veiculo as any).id,
      motorista_id: user.id,
      tipo:         formProblema.tipo,
      urgencia:     formProblema.urgencia,
  ...
  const todosHandled        = (legAtual?.passengers.length ?? 0) > 0 &&
                              legAtual!.passengers.every(p => p.status !== 'aguardando')
  ...
  if (!loading && !plan) return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center gap-4">
        ...
        <button onClick={() => carregar()} className="mt-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm">
          🔄 Verificar novamente
        </button>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center gap-4">
        ...
        <p className="text-gray-400 text-sm">
          Você concluiu {legs.length} viagem(ns) hoje.<br />
          Bom descanso! 👏
        </p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col pb-24">
      ...
      <div className="flex-1 p-4 space-y-4">
        ...
        {/* ── ESTADO: aguardando (início ou entre viagens) ── */}
        {legAtual?.status === 'aguardando' && (
          <>
            <div className="bg-white rounded-2xl shadow p-5 text-center space-y-3">
              <div className="text-gray-400 text-sm">
                {legIdx === 0 ? 'Primeira saída em' : 'Próxima saída em'}
              </div>
              {legAtual.est_departure_at ? (
                <Countdown targetIso={legAtual.est_departure_at} />
              ) : (
                <div className="text-2xl font-bold text-blue-700">
                  {fmtHora(null, legAtual.horario_saida)}
                </div>
              )}
              <div className="text-sm text-gray-600">
                🏥 {legAtual.hospital.nome}
                {legAtual.hospital.cidade && ` — ${legAtual.hospital.cidade}`}
              </div>
              {legAtual.est_outbound_min && (
                <div className="text-xs text-gray-400">
                  ~{legAtual.est_outbound_min} min de viagem
                </div>
              )}
              <button onClick={() => setLegStatus(legAtual.id, 'em_andamento')} disabled={atualizando}
                className="w-full mt-2 bg-blue-600 text-white py-3 rounded-xl font-bold text-base
            ...
            <div className="bg-white rounded-2xl shadow p-4">
              <h3 className="font-semibold text-gray-700 text-sm mb-3">
                👥 {legAtual.passengers.length} parada(s) nesta viagem
              </h3>
              <div className="space-y-2">
                {legAtual.passengers.map((p, i) => (
                  <div key={p.id} className="flex items-start gap-3 text-sm">
                    <span className="w-5 h-5 rounded-full bg-gray-100 text-gray-500 text-xs
                    ...
                    <div>
                      ...
                      {p.est_pickup_at && (
                        <div className="text-xs text-blue-600">🕐 {fmtHora(p.est_pickup_at)}</div>
                      )}
                    </div>
                  </div>
                ))}
        ...
        {legAtual?.status === 'em_andamento' && (
          <>
              ...
              <div className="bg-blue-600 text-white rounded-2xl shadow p-5 space-y-3">
                ...
                {(passeiroAtual.paciente.endereco || passeiroAtual.paciente.bairro) && (
                  <div className="text-blue-100 text-sm">
                    📍 {[passeiroAtual.paciente.endereco, passeiroAtual.paciente.bairro].filter(Boolean).join(', ')}
                  </div>
                )}
                {passeiroAtual.est_pickup_at && (
                  <div className="text-blue-200 text-sm">🕐 Estimado: {fmtHora(passeiroAtual.est_pickup_at)}</div>
                )}
                <div className="flex gap-2 mt-2">
                  <a href={mapsUrl(passeiroAtual.paciente)} target="_blank" rel="noopener noreferrer"
                    className="flex-1 bg-white/20 text-white text-center py-2.5 rounded-xl text-sm font-medium
                  ...
                  <button onClick={() => setPassStatus(passeiroAtual.id, 'embarcou')} disabled={atualizando}
                    className="flex-1 bg-green-500 text-white py-2.5 rounded-xl text-sm font-bold
                               hover:bg-green-600 active:scale-95 transition-transform disabled:opacity-50">
                    ✅ Embarcou
                  </button>
                  <button onClick={() => setPassStatus(passeiroAtual.id, 'ausente')} disabled={atualizando}
                    className="flex-1 bg-red-400 text-white py-2.5 rounded-xl text-sm font-bold
                               hover:bg-red-500 active:scale-95 transition-transform disabled:opacity-50">
                    ✗ Ausente
                  </button>
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow p-4">
                  ...
                  return (
                    <div key={p.id}
                      className={`flex items-start gap-3 p-3 rounded-xl transition-colors
                        ...
                        ${p.status === 'embarcou' ? 'opacity-60' : ''}
                        ${p.status === 'ausente'  ? 'opacity-40' : ''}`}>
                      <span className={`w-6 h-6 rounded-full text-xs flex items-center justify-center
                                       flex-shrink-0 mt-0.5 font-bold
                        ${p.status === 'embarcou' ? 'bg-green-100 text-green-700'
                        : p.status === 'ausente'  ? 'bg-red-100 text-red-600'
              ...
              {/* Botão cheguei ao hospital */}
              {todosHandled && (
                <button onClick={() => setLegStatus(legAtual.id, 'no_hospital')} disabled={atualizando}
                  className="w-full mt-4 bg-red-600 text-white py-3 rounded-xl font-bold text-base
                             hover:bg-red-700 active:scale-95 transition-transform disabled:opacity-50">
                  🏥 Cheguei ao Hospital
                </button>
              )}
            </div>
        ...
        {legAtual?.status === 'no_hospital' && (
          <div className="bg-white rounded-2xl shadow p-5 space-y-4">
            <div className="text-center">
              ...
              {legAtual.est_hospital_at && (
                <div className="text-sm text-gray-400">Chegada: {fmtHora(legAtual.est_hospital_at)}</div>
              )}
            </div>
              ...
              <div className="space-y-1.5">
                ...
                {legAtual.passengers.filter(p => p.status === 'embarcou').map(p => (
                  <div key={p.id} className="flex items-center gap-2 text-sm bg-green-50 rounded-lg px-3 py-2">
                    ...
                    <span className="text-gray-700">{p.paciente.nome}</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="space-y-1.5">
                ...
                {legAtual.passengers.filter(p => p.status === 'ausente').map(p => (
                  <div key={p.id} className="flex items-center gap-2 text-sm bg-red-50 rounded-lg px-3 py-2">
                    ...
                    <span className="text-gray-500">{p.paciente.nome}</span>
                  </div>
                ))}
              </div>
            )}
              ...
              {legAtual.est_return_at && (
                <>
                  ...
                  <div className="text-3xl font-bold text-blue-700 font-mono">
                    {fmtHora(legAtual.est_return_at)}
                  </div>
                </>
              )}
              {legAtual.est_return_min && (
                <div className="text-sm text-gray-400">~{legAtual.est_return_min} min de viagem</div>
              )}
              <button onClick={() => setLegStatus(legAtual.id, 'concluida')} disabled={atualizando}
                className="w-full mt-2 bg-green-600 text-white py-3 rounded-xl font-bold text-base
                           hover:bg-green-700 active:scale-95 transition-transform disabled:opacity-50">
                🏠 Cheguei de Volta
              </button>
            </div>

            {/* Preview da próxima viagem */}
            {proxLeg && (
              <div className="bg-blue-50 rounded-2xl p-4 space-y-2 border border-blue-100">
                <div className="text-xs text-blue-500 font-semibold uppercase tracking-wide">
                  Próxima viagem (Viagem {legIdx + 2})
                </div>
                <div className="font-semibold text-gray-800 text-sm">🏥 {proxLeg.hospital.nome}</div>
                <div className="text-xs text-gray-500">
                  Saída: {fmtHora(proxLeg.est_departure_at, proxLeg.horario_saida)}
                  {proxLeg.est_outbound_min && ` · ~${proxLeg.est_outbound_min} min`}
                </div>
          ...
          <div className="bg-white rounded-2xl shadow p-4">
            ...
            {legs.filter(l => l.status === 'concluida').map((l, i) => (
              <div key={l.id} className="flex items-center gap-2 text-sm py-1.5 border-b last:border-0">
                ...
                {l.est_return_at && (
                  <span className="ml-auto text-xs text-gray-400">{fmtHora(l.est_return_at)}</span>
                )}
              </div>
            ))}
          </div>
      ...
      {/* ── Botão fixo no rodapé: reportar problema ── */}
      {plan && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-3">
          <button onClick={() => setModalProblema(true)}
            className="w-full border border-orange-300 text-orange-600 py-2.5 rounded-xl text-sm font-medium
                       hover:bg-orange-50 active:scale-95 transition-transform">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end">
          <div className="bg-white w-full rounded-t-2xl p-5 space-y-4">
            ...
            <div>
              ...
              <select value={formProblema.tipo}
                onChange={e => setFormProblema(f => ({ ...f, tipo: e.target.value }))}
                ...
                {(['baixa','normal','alta','critica'] as const).map(u => (
                  <button key={u} onClick={() => setFormProblema(f => ({ ...f, urgencia: u }))}
                    className={`py-2 rounded-xl text-xs font-medium capitalize transition-colors
                      ${formProblema.urgencia === u
                        ? u === 'critica' ? 'bg-red-600 text-white'
                        : u === 'alta'    ? 'bg-orange-500 text-white'
                        : u === 'normal'  ? 'bg-yellow-400 text-white'
            ...
            <div>
              ...
              <textarea value={formProblema.descricao} rows={3}
                onChange={e => setFormProblema(f => ({ ...f, descricao: e.target.value }))}
                placeholder="Descreva o problema…"
                className="w-full border rounded-xl px-3 py-2.5 text-sm resize-none" />
            </div>
            <button onClick={enviarProblema} disabled={enviandoProbl || !formProblema.descricao.trim()}
